...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      Provider.of<MovieDetailCubit>(context, listen: false)
        ..getDetailMovie(widget.id);
      Provider.of<RecommendationMoviesCubit>(context, listen: false)
        ..getRecommendationMovies(widget.id);
    ...
    return Scaffold(
      body: BlocBuilder<MovieDetailCubit, MovieDetailState>(
        builder: (context, data) {
          ...
          var movieDetail = data.movieDetail;
          if (data.state == RequestState.Loading) {
            return Center(
              child: CircularProgressIndicator(),
            );
          } else if (data.state == RequestState.Loaded && movieDetail != null) {
            return BlocBuilder<RecommendationMoviesCubit, RecommendationState>(
                builder: (context, data) {
              return SafeArea(
                child: DetailContent(
                  movieDetail,
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                if (!widget.isAddedWatchlist) {
                                  Provider.of<WatchlistMovieBloc>(context, listen: false)
                                    ..add(AddMovieToWatchlistEvents(widget.movie));
                                } else {
                                  Provider.of<WatchlistMovieBloc>(context, listen: false)
                                    ..add(RemoveMovieFromWatchlistEvents(widget.movie));
                                }
